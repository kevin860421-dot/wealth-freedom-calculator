...
import { Q10_DARK_CARD } from "./quick10-theme";

export { Q10_DARK_CARD };
export const Q10_DARK_SHELL = "rounded-xl border border-slate-700 bg-[#0f172a] p-2.5 sm:p-3";
...
import { amountFromInvertedRange, clampRangeAmount, invertedFillPct, invertedRangeDisplay } from "@/app/components/quick-inverted-range";
...
export function StressSlider({
  ...
  const sliderDisplay = invertedRangeDisplay(amount, min, max);
    ...
    <div className="space-y-1.5">
      ...
      <input
        ...
        onChange={(e) => {
          ...
          onChange(amountFromInvertedRange(raw, min, max));
        }}
        style={{ "--fill-pct": fillPct } as CSSProperties}
        className={`h-2.5 w-full cursor-pointer appearance-none rounded-lg bg-[#0f172a] ${accentClass}`}
      />
    ...
    <div className="space-y-1.5">
      ...
      <div className="flex items-center gap-2">
        <button
          type="button"
          onClick={() => onBump(step)}
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-slate-600 bg-slate-800 text-[18px] font-black leading-none text-sky-300 shadow-[inset_0_1px_0_rgba(148,163,184,0.12)] transition active:scale-95 hover:border-sky-500 hover:bg-slate-700"
          aria-label="增加"
        >
          +
        </button>
        <div className="flex min-w-0 flex-1 items-center rounded-md border border-slate-600 bg-[#0b1220] px-2">
          <input
            ...
            className="min-w-0 w-full border-0 bg-transparent py-2 text-center text-[16px] font-black tabular-nums text-slate-100 outline-none placeholder:text-slate-500 focus:border-sky-400"
            inputMode="decimal"
          />
