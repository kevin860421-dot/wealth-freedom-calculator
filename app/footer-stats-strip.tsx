...
export function FooterStatsStrip() {
  ...
  const lineShareUrl = `https://social-plugins.line.me/lineit/share?url=${encodeURIComponent(PROD_URL)}`;
  const fbShareUrl = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(PROD_URL)}`;
  const threadsShareUrl = `https://www.threads.net/intent/post?text=${encodeURIComponent(`快來試算你的財富自由還需要多久 👇\n${PROD_URL}`)}`;
          ...
          <div className={`${styles.cardBody} ${styles.shareBody}`}>
            ...
            <div className={styles.platformRow}>
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className={`${styles.platformBtn} ${styles.platformIg} ${styles.mobileOnly}`}
              >
                <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
