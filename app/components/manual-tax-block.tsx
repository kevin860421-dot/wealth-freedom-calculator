...
/** 稅金／級距／年收入列（手機與桌機共用） */
export function ManualTaxTopControls(props: ManualTaxBlockProps) {
  const {
    applyTaxInTable,
    setApplyTaxInTable,
    taxBracketRate,
...
export type ManualTaxEstimatePriceRowProps = ManualTaxBlockProps & {
  /** 外層容器 class（手機版排版） */
  rowClassName?: string;
  /** 合併進 input 的樣式（例如手機加寬） */
  priceInputStyle?: CSSProperties;
  /** 手機：隱藏「總股價」字樣、輸入與股利分行 */
  stacked?: boolean;
};

/** 總股價輸入 + 預估當期股利（與試算同步） */
export function ManualTaxEstimatePriceRow(props: ManualTaxEstimatePriceRowProps) {
  const {
    rowClassName,
    priceInputStyle,
    stacked,
  ...
  if (!deductionEstimate) return null;
  ...
  const inputEl = (
    <input
      ...
      style={{ ...inputStyle, boxSizing: "border-box", width: 120, height: 24, ...priceInputStyle }}
    />
  );
  ...
  if (stacked) {
    return (
      <div className={rowClassName}>
        {inputEl}
        <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 8, fontSize: 12, color: "#d1d5db" }}>
          <span>元</span>
...
/** 級距與課稅方式一行 */
export function ManualTaxEstimateBracketRow(props: ManualTaxBlockProps) {
  const { deductionEstimate } = props;
  if (!deductionEstimate) return null;
  return (
...
/** 稅金試算、公式、股數推導（完整技術區） */
export function ManualTaxEstimateTaxAndSharesColumn(props: ManualTaxBlockProps) {
  const { deductionEstimate, sharesForTaxThreshold, sharesForCreditCap80k, selectedEtfInfo, taxThreshold } = props;
  if (!deductionEstimate) return null;
  return (
...
/** 試算明細（供桌機直接顯示、手機收合區舊版） */
export function ManualTaxEstimateDetails(props: ManualTaxBlockProps) {
  const { estimateMarginTop = 0, deductionEstimate } = props;
    ...
    deductionEstimate && (
      <div className={styles.estimateBlock} style={estimateMarginTop ? { marginTop: estimateMarginTop } : undefined}>
        ...
        <div style={{ display: "grid", gap: 3 }}>
          ...
          <ManualTaxEstimatePriceRow {...props} />
...
export function ManualTaxBlock(props: ManualTaxBlockProps) {
  ...
  return (
    <>
      ...
      <ManualTaxEstimateDetails {...props} estimateMarginTop={estimateMarginTop} />
    </>
  );
}
