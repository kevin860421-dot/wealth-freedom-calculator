...
type ExcelPickVariant = "formula" | "plain";
...
export function Quick11ExcelWizardModal({ open, onClose, snapshotRef }: Quick11ExcelWizardModalProps) {
  ...
  const [selectedPick, setSelectedPick] = useState<ExcelPickVariant>("formula");
  const [portalReady, setPortalReady] = useState(false);
  const shareFileRef = useRef<File | null>(null);
  const shareUnlockTimerRef = useRef<number | null>(null);
  const panelScrollRef = useRef<HTMLDivElement>(null);
  /** 捲動錨點：四步驟說明區（stepsLead）頂部，非第一步按鈕列 */
  const stepsIntroAnchorRef = useRef<HTMLDivElement>(null);
  const scrollToStepsIntroAfterExpandRef = useRef(false);
  ...
  useEffect(() => {
    if (!open) {
      ...
      setSelectedPick("formula");
      return;
    }
    refresh();
    const prog = readQuick11WizardProgress();
    const hasStarted =
      prog.screenshotDone || prog.shareDone || prog.copyDone || prog.fbOpened;
    setStepsExpanded(hasStarted);
  ...
  const scrollPanelToStepsIntro = useCallback(() => {
    const panel = panelScrollRef.current;
    const anchor = stepsIntroAnchorRef.current;
    if (!panel || !anchor) return;
    const panelTop = panel.getBoundingClientRect().top;
    const anchorTop = anchor.getBoundingClientRect().top;
    const nextTop = panel.scrollTop + (anchorTop - panelTop) - 4;
    panel.scrollTo({ top: Math.max(0, nextTop), behavior: "smooth" });
  }, []);

  useEffect(() => {
    if (!stepsExpanded || !scrollToStepsIntroAfterExpandRef.current) return;
    scrollToStepsIntroAfterExpandRef.current = false;
    requestAnimationFrame(() => requestAnimationFrame(scrollPanelToStepsIntro));
  }, [stepsExpanded, scrollPanelToStepsIntro]);
  ...
  const onNoFormulaDownload = useCallback(() => {
    setSelectedPick("plain");
    window.location.assign(QUICK11_EXCEL_PUBLIC_DOWNLOAD_PATH);
  }, []);

  /** 完整有公式：下載檔案 → 展開四步驟並捲至說明區頂部（非直連下載） */
  const onFormulaDownloadClick = useCallback(() => {
    setSelectedPick("formula");
    if (stepsExpanded) {
      requestAnimationFrame(() => requestAnimationFrame(scrollPanelToStepsIntro));
      return;
    }
    scrollToStepsIntroAfterExpandRef.current = true;
    setStepsExpanded(true);
  }, [stepsExpanded, scrollPanelToStepsIntro]);

  const formulaSelected = selectedPick === "formula";
  const plainSelected = selectedPick === "plain";

  const portalTarget = portalReady && typeof document !== "undefined" ? document.body : null;

  const wizardSteps = (
    <>
      <div ref={stepsIntroAnchorRef} className={`${styles.stepsLead} scroll-mt-1`}>
        <p className={styles.stepsLeadTitle}>如何領取完整有公式 Excel？</p>
        <p className={styles.stepsLeadBody}>
          請先<strong>截圖試算結果</strong>，再<strong>分享至 LINE 或 FB</strong>，依序複製專屬密碼並至粉專留言，即可取得檔案。
        </p>
        <ol className={styles.stepsLeadList}>
          <li>截圖你的試算畫面</li>
          <li>分享截圖至 LINE／FB</li>
          <li>複製專屬解鎖密碼</li>
          <li>至粉專留言領取 Excel</li>
      ...
      <div className={`${styles.stepDivider} mt-3 pt-1`}>
    ...
    <AnimatePresence>
      {open ? (
            <motion.div
              ...
              className={`${styles.shell} ${styles.sheetRoot} border border-slate-600/80 bg-[#161b22] text-slate-100`}
            >
              <button
                type="button"
                aria-label="關閉視窗"
              ...
              <div className={`${styles.panelBody} px-3.5 pt-3.5 sm:px-4`}>
                <Quick11Excel24hCountdown size="large" hidden />
                ...
                <div className={styles.pickStack}>
                  ...
                  >
                    ...
                    <ul className={styles.pickFeatureList}>
                      <li className={`${styles.pickFeatureItem} ${plainSelected ? styles.pickFeatureItemFeatured : ""}`}>
                        <span className={`${styles.pickCheck} ${plainSelected ? styles.pickCheckFeatured : styles.pickCheckPlain}`} aria-hidden>✓</span>
                        僅保留計算結果與資料
                      </li>
                      <li className={`${styles.pickFeatureItem} ${plainSelected ? styles.pickFeatureItemFeatured : ""}`}>
                        <span className={`${styles.pickCheck} ${plainSelected ? styles.pickCheckFeatured : styles.pickCheckPlain}`} aria-hidden>✓</span>
                    ...
                    <button
                      ...
                      className={plainSelected ? styles.pickCtaFeatured : styles.pickCtaPlain}
                    >
                      <span aria-hidden>⬇</span>
                      下載檔案
                    </button>
                  ...
                  <article
                    role="button"
                    tabIndex={0}
                    aria-pressed={formulaSelected}
                    onClick={() => setSelectedPick("formula")}
                    onKeyDown={(e) => {
                      if (e.key === "Enter" || e.key === " ") {
                        e.preventDefault();
                        setSelectedPick("formula");
                    ...
                    className={`${styles.pickCard} ${formulaSelected ? styles.pickCardFeatured : styles.pickCardPlain}`}
                  >
                    {formulaSelected ? <span className={styles.pickBadge}>⭐ 推薦使用</span> : null}
                    <div className={`${styles.pickCardHead} ${formulaSelected ? "mt-2" : ""}`}>
                      <div className="min-w-0">
                    ...
                    <ul className={styles.pickFeatureList}>
                      <li className={`${styles.pickFeatureItem} ${formulaSelected ? styles.pickFeatureItemFeatured : ""}`}>
                        <span className={`${styles.pickCheck} ${formulaSelected ? styles.pickCheckFeatured : styles.pickCheckPlain}`} aria-hidden>✓</span>
                        可修改數字與假設條件
                      </li>
                      <li className={`${styles.pickFeatureItem} ${formulaSelected ? styles.pickFeatureItemFeatured : ""}`}>
                        <span className={`${styles.pickCheck} ${formulaSelected ? styles.pickCheckFeatured : styles.pickCheckPlain}`} aria-hidden>✓</span>
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        onFormulaDownloadClick();
                      }}
                      className={formulaSelected ? styles.pickCtaFeatured : styles.pickCtaPlain}
                    >
                      <span aria-hidden>⬇</span>
                      下載檔案
                    </button>
                ...
                {stepsExpanded ? wizardSteps : null}
              </div>
              </div>
  ...
  return (
    <>
      {createPortal(wizardLayer, portalTarget)}
      {previewLayer ? createPortal(previewLayer, portalTarget) : null}
    </>
  );
}
