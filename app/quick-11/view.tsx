...
export function QuickCalculator11View({ embedPreset }: { embedPreset?: Quick11EmbedPreset } = {}) {
  const p = embedPreset;
  const [loanAmount, setLoanAmount] = useState(p?.loanAmount ?? 12_000_000);
  const [loanAmountText, setLoanAmountText] = useState(formatMoney(p?.loanAmount ?? 12_000_000));
  const [annualRate, setAnnualRate] = useState(p?.annualRate ?? 2.2);
  const [annualRateText, setAnnualRateText] = useState(String(p?.annualRate ?? 2.2));
  const [loanYears, setLoanYears] = useState(p?.loanYears ?? 30);
  const [loanYearsText, setLoanYearsText] = useState(String(p?.loanYears ?? 30));
  const [monthlyIncome, setMonthlyIncome] = useState(p?.monthlyIncome ?? 80_000);
  const [monthlyIncomeText, setMonthlyIncomeText] = useState(formatMoney(p?.monthlyIncome ?? 80_000));

  const [method, setMethod] = useState<LoanMethod>(p?.method ?? "equalPrincipal");
  const [currentPage, setCurrentPage] = useState(p?.initialPage ?? 0);
  const [pageDirection, setPageDirection] = useState(0);
  const [extraMonthlyPayment, setExtraMonthlyPayment] = useState(p?.extraMonthlyPayment ?? 10_000);
  const [earlyRepayMethod, setEarlyRepayMethod] = useState<LoanMethod>("annuity");
  const [lumpSumAmount, setLumpSumAmount] = useState(p?.lumpSumAmount ?? 1_000_000);
  const [lumpSumText, setLumpSumText] = useState(formatMoney(p?.lumpSumAmount ?? 1_000_000));
  ...
  const loanPresetActions: LoanPresetAction[] = [
    { key: "scooter", icon: "🛵", label: "機車貸", amount: 100_000, annualRate: 12, years: 4, monthlyIncome: 35_000 },
    ...
    { key: "mortgage", icon: "🏠", label: "房貸", amount: 12_000_000, annualRate: 2.2, years: 30, monthlyIncome: 120_000 },
    { key: "student", icon: "🎓", label: "學貸", amount: 400_000, annualRate: 1.9, years: 10, monthlyIncome: 45_000 },
  ...
  return (
    <main className={`min-h-screen px-3 py-4 ${isLight ? "bg-white text-slate-900" : "bg-[#020817] text-slate-100"}`}>
              ...
              >
                <InputField
                  ...
                  onTextChange={(next) => {
                    ...
                    setLoanAmount(parseAndClamp(next, loanAmount, 500_000, 50_000_000, true));
                  }}
                  onCommit={(next) => {
                    const normalized = parseAndClamp(next, loanAmount, 500_000, 50_000_000, true);
                    setLoanAmount(normalized);
                    setLoanAmountText(formatMoney(normalized));
                  }}
                  onBump={(delta) => {
                    const next = Math.min(50_000_000, Math.max(500_000, loanAmount + delta));
                    setLoanAmount(next);
                  ...
                  {currentPage === 0 ? (
                    <div className="space-y-2">
                      ...
                      <div className="rounded-lg border border-slate-700 bg-slate-900/70 p-2">
                        <p className="text-[10px] tracking-[0.04em] text-slate-400">快速導覽</p>
                        <p className="mt-1 text-sm font-semibold text-slate-200">
                          首頁固定在左側；右側分頁可左右滑動：本息均攤、本金平均、提前還款、大額還款、延遲還款代價、各種貸款 vs 存股、風險模擬、財富翻轉。
                        </p>
                      </div>
                    </div>
                  ) : null}
                            ...
                            <button
                              ...
                              className={`rounded-lg border px-3 py-1.5 text-[13px] font-black transition ${
                                isLight
                                  ...
                                  : "border-slate-600 bg-slate-900/60 text-slate-100 hover:border-amber-400 hover:bg-slate-800"
                              }`}
                            >
                              {b.label}
                            </button>
...
function MiniSettingsHeader(props: { isLight: boolean }) {
  ...
  return (
    ...
    >
      <div className={`grid grid-cols-2 gap-x-2 gap-y-1 text-[13px] font-semibold tracking-[0.04em] ${isLight ? "text-slate-600" : "text-slate-300"}`}>
