...
/** 車貸繳款中：月繳轉為本金／自有累積之示意比例（與第六台一致，不含車價折舊） */
export const CAR_LOAN_EQUITY_SHARE = 0.55;
...
/**
 * 車貸 path：1..N 年僅示意累積；N 年結清後「累積示意 + 每月 M」全數依複利滾入股市。
 */
export function carLoanVsStockPath(
  monthly: number,
  loanYears: number,
  ...
  const N = Math.round(clampNum(loanYears, YEARS_MIN, YEARS_MAX));
  const y = Math.max(0, calendarYears);
  if (y <= N) {
    return Math.round(m * 12 * y * equityShare);
  }
  const lumpAtLoanEnd = Math.round(m * 12 * N * equityShare);
