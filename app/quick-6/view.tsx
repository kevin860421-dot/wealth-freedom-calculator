...
export function QuickCalculator6View() {
  ...
  const series = useMemo(() => {
    ...
    const mortgage = yearsList.map((yy) =>
      mortgageVsStockPath(m, yearsClamped, yy, MORTGAGE_EQUITY_SHARE, INVEST_ANNUAL_PCT),
    );
    return { stock, mortgage };
  ...
  const milestoneGaps = useMemo(() => {
    ...
    return yearMarkers.map((yy) => {
      ...
      const mort = mortgageVsStockPath(m, yearsClamped, yy, MORTGAGE_EQUITY_SHARE, INVEST_ANNUAL_PCT);
      const delta = stock - mort;
      return { year: yy, stock, mortgage: mort, delta };
    });
    ...
    >
      <style jsx global>{`
        ...
        *::after {
        ...
        @keyframes quick6TitleGradientShift {
          0% {
        ...
        .quick6-title-gradient {
          ...
          animation: quick6TitleGradientShift 7s ease-in-out infinite alternate;
        }
      `}</style>
      ...
      <div style={{ width: "100%", maxWidth: 400, margin: "0 auto", boxSizing: "border-box", minWidth: 0, overflowX: "hidden" }}>
        <div style={{ marginBottom: 14 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
            <div className="quick-brand-gold-shimmer" style={{ fontSize: 20, fontWeight: 900, opacity: 0.95, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", ["--quick-brand-duration" as string]: "3.1s" }}>
              財富自由計算機
            </div>
            <button
              type="button"
          ...
          <div
            className="quick6-title-gradient"
            style={{
              fontSize: 28,
              fontWeight: 950,
              marginTop: 10,
        ...
        >
          <div style={{ display: "grid", gap: 10 }}>
            ...
            <QuickDualLineChart
              years={yearsList}
              seriesA={series.mortgage}
              seriesB={series.stock}
              legendA={`房貸情境（繳款中示意 ×${MORTGAGE_EQUITY_SHARE}；結清後全進股市）`}
            ...
            <div style={{ padding: 12, borderRadius: 14, background: "rgba(0,0,0,0.20)", border: "1px solid rgba(255,255,255,0.12)" }}>
              <div style={{ fontSize: 16, fontWeight: 900, opacity: 0.95 }}>固定里程碑·差距（股市 − 房貸路徑）</div>
            ...
            <QuickBottomCtaStack quickId={6} style={{ marginTop: 4 }} />
